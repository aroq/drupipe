...
//            this.action.params.jobName = "${projects[i]}/seed"
    ...
    def parseProjects(def projects, String param, String tag, String drupipeEnv) {
        ...
        for (project in projects) {
            if (project.value?.params?.containsKey(param) && project.value.params[param].contains(tag) && project.value.params['jenkinsServers'] && project.value.params['jenkinsServers'].contains(drupipeEnv)) {
                result << project.key
            }
        }
        result.join(',')
